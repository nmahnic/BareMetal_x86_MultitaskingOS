...
__attribute__(( section(".tarea1"))) __sumaDigitosT1(dword *src,dword *dst, dword *limite, dword *hexa){
    //asm("xchg %bx,%bx");
    byte i;
    src = src + *limite - 4;
    byte valores1[] = {(byte)*src , (byte)(*src >> 8) , (byte)(*src >> 16) , (byte)(*src >> 24) };
    src++;
    byte valores2[] = {(byte)*src , (byte)(*src >> 8) , (byte)(*src >> 16) , (byte)(*src >> 24) };
    dword res = *dst;
    for(i=0;i<4;i++){
        if(valores1[i] < 0x30){ 
            valores1[i]=0;
        }else if(valores1[i] > 0x39){
            valores1[i] =valores1[i]-0x37;
        }else{
            valores1[i] =valores1[i]-0x30;
        }
        if(valores2[i] < 0x30){ 
            valores2[i]=0;
        }else if(valores2[i] > 0x39 ){
            valores2[i] =valores2[i]-0x37;
        }else{
            valores2[i] =valores2[i]-0x30;
        }
    }
    //asm("xchg %bx,%bx");
    res += valores1[0]+valores1[1]+valores1[2]+valores1[3];
    //*dst = res;
    //asm("xchg %bx,%bx");
    res += valores2[0]+valores2[1]+valores2[2]+valores2[3];
    *dst = res;
}
...
__attribute__(( section(".tarea2"))) __sumaDigitosT2(dword *src,dword *dst, dword *limite, dword *hexa){
    //asm("xchg %bx,%bx");
    byte i;
    src = src + *limite - 4;
    byte valores1[] = {(byte)*src , (byte)(*src >> 8) , (byte)(*src >> 16) , (byte)(*src >> 24) };
    ...
    //asm("xchg %bx,%bx");
    res += valores1[0]*1000+valores1[1]*100+valores1[2]*10+valores1[3];
    //*dst = res;
    //asm("xchg %bx,%bx");
    res += valores2[0]*1000+valores2[1]*100+valores2[2]*10+valores2[3];
    *dst = res;
}