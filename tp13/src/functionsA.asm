...
__init_myTSS:
    ...
    mov [eax + 24], dword(0)                ; <- EBP
    mov [eax + 28], dword(ecx)              ; <- ESP3
    mov [eax + 32], dword(DS_SEL)           ; <- GS
    mov [eax + 36], dword(DS_SEL)           ; <- FS
    mov [eax + 40], dword(DS_SEL)           ; <- SS3
    mov [eax + 44], dword(DS_SEL)           ; <- ES
    mov [eax + 48], dword(DS_SEL)           ; <- DS
    mov [eax + 52], dword(ebx)              ; <- CR3
    mov [eax + 56], dword(CS_SEL_32)        ; <- CS
    mov [eax + 60], dword(edx)              ; <- ESP0
    mov [eax + 64], dword(DS_SEL)           ; <- SS0
