...
;xchg bx,bx
 ...
 ;------- Tarea 1 BSS (BSS1_lineal = 0x0130 1000 -> BSS1 = 0x0030 1000) ------
  ...
  ;------- Tarea 1 DATOS (DATA1_lineal = 0x0130 2000 -> DATOS1 = 0x0030 2000) ------
    ...
    ;------- PAGESTABLES_T1 (PAGETABLES_T1 = 0x0130 3000 -> DTP_1 = 0x0030 3000) ------
    mov eax, __TP4                           ; Esta tabla está en __DTPK + 0x4000 (tabla num 3)
    add eax, PAG_RW_W_US_SUP                 ; ATTR de la DTE
    mov dword [__DTPK + 0x004*4], eax        ; DTP entry (offset 0x004)
    mov ebx, __TP4 + 0x303*4                 ; TP entry (offset 0x303)
    mov eax, 0x00303000                      ; Dir de la pagina  
    or eax, PAG_RW_W_US_SUP                  ; ATTR de la PTE
    mov esi, ebx                  
    mov ecx,0x6                              ; un DTP + 5 tablas
 ciclo2:
 ...
 ; =================================Paginas para DTP_T1 ======================================
    ...
    ;------- PAGESTABLES_T1 (PAGETABLES_T1 = 0x0130 3000 -> DTP_1 = 0x0030 3000) ------
    mov eax, __TP4_T1                           ; Esta tabla está en __DTP_T1 + 0x4000 (tabla num 3)
    add eax, PAG_RW_W_US_SUP                 ; ATTR de la DTE
    mov dword [__DTP_T1 + 0x004*4], eax        ; DTP entry (offset 0x004)
    mov ebx, __TP4_T1 + 0x303*4                 ; TP entry (offset 0x303)
    mov eax, 0x00303000                      ; Dir de la pagina  
    or eax, PAG_RW_W_US_SUP                  ; ATTR de la PTE
    mov esi, ebx                  
    mov ecx,0x6                              ; un DTP + 5 tablas
